...
export function useO2TaxAnalytics(startDate: Date, endDate: Date) {
  ...
  const firstEntryByCardAndIndicator = useMemo(() => {
    ...
    for (const card of historyToUse) {
      ...
      // Cards desqualificados como MQL (motivo de perda) são suprimidos de todo o funil
    ...
    return firstEntries;
  }, [cards, fullHistory, excludedMqlIds]);
  ...
  const getCardsForIndicator = useMemo(() => {
    return (indicator: IndicatorType): O2TaxCard[] => {
      ...
      if (indicator === 'rm') {
        ...
        const byTitleMonth = new Map<string, O2TaxCard>();
        ...
        console.log(`[O2 TAX Analytics] getCardsForIndicator(rm): ${result.length} → ${deduped.length} (dedup titulo|mês)`);
        return deduped;
      }
