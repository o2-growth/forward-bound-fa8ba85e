...
export function VisaoGeralCS({ clientes, cfos, alertas, npsScore, mrrBase, onNavigateToAlertas, operacao }: VisaoGeralCSProps) {
  ...
  const clientesByProduto = useMemo(() => {
    const map: Record<string, { count: number; mrr: number; pontual: number; clientes: JornadaCliente[] }> = {};
    activeClientes.forEach(c => {
      ...
      unique.forEach(produto => {
        if (!map[produto]) map[produto] = { count: 0, mrr: 0, pontual: 0, clientes: [] };
        map[produto].count++;
        map[produto].mrr += c.mrr;
        map[produto].pontual += c.pontual || 0;
  ...
  const kpis: { key: KpiDialogType; label: string; value: string; icon: typeof Users; color: string; tooltip: string }[] = [
    ...
    { key: 'mrr', label: 'MRR Base', value: formatCurrency(mrrBase), icon: DollarSign, color: 'text-green-600', tooltip: 'MRR Base oficial vindo da Oxy Finance (tabela mrr_base_monthly). Reflete o mês final do período selecionado ou o mês mais recente disponível. Fonte: Oxy Finance.' },
