...
export function VisaoGeralCS({ clientes, cfos, alertas, npsScore, mrrBase, onNavigateToAlertas, operacao }: VisaoGeralCSProps) {
  ...
  const kpis: { key: KpiDialogType; label: string; value: string; icon: typeof Users; color: string; tooltip: string }[] = [
    ...
    { key: 'mrr', label: 'MRR Base', value: formatCurrency(mrrBase), icon: DollarSign, color: 'text-green-600', tooltip: 'Soma (Valor CFOaaS + Valor OXY) de clientes ativos no fim do período selecionado. Respeita filtros de CFO, Produto e Data. Exclui produtos pontuais. Fonte: Pipefy' },
